...
flowchart TD
  ...
  F --> G
  G --> H["Действие клетки и до двух бизнес-действий"]
  H --> I["Завершить ход"]
```
...
1. Инициатор выбирает продажу ресурса, встречный обмен или субпоставку в активный контракт.
2. Предложение сохраняется со статусом `pending`.
3. Покупатель принимает или отклоняет предложение.
4. При принятии повторно проверяются ресурсы обеих сторон, деньги, склады и актуальная потребность контракта.
5. Все ресурсы и деньги передаются одной операцией, предложение получает статус `accepted`.
6. При субпоставке ресурс сразу увеличивает `contract.filled`, а поставщик получает +1 влияние.
7. Непринятые предложения истекают при смене торгового дня.
...
- игрок стоит на клетке `claim`;
- действие клетки еще не использовано.

Игрок обязан выбрать один вариант:

1. Заплатить 2 млн и сохранить оба бизнес-действия.
2. Потерять одно бизнес-действие текущего хода без денежной оплаты.
3. Заплатить 2 млн и увеличить срок выбранного активного контракта на 1 торговый день.

До выбора завершение хода заблокировано.
